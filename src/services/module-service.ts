...
const templateRepos: {[type: string]: TemplateRepo} = {
  gitops: {template_owner: 'cloud-native-toolkit', template_repo: 'template-gitops-module'},
  terraform: {template_owner: 'cloud-native-toolkit', template_repo: 'template-gitops-module'},
}

const buildNameAndDescription = (repoType: string, baseName: string, provider?: string): {name: string, description: string} => {
  return {
...
const buildName = (repoType: string, baseName: string, provider?: string): string => {
  if (repoType === 'gitops') {
...
const buildDescription = (repoType: string, baseName: string, provider?: string): string => {
  if (repoType === 'gitops') {
...
export class ModuleService {
  ...
  async run({octokit, repoType, baseName, provider, owner}: ModuleServiceParams): Promise<{repoUrl: string}> {
    ...
    const repo: ModuleRepo = await ModuleRepo.createFromTemplate(octokit, templateRepo, owner, name, description);
